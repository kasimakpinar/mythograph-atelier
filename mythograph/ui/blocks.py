    ...
    with gr.Blocks(title=APP_TITLE, css=css, elem_id="ma-shell", theme=gr.themes.Base()) as demo:
        ...
        with gr.Group(elem_id="ma-start-panel") as start_panel:
            ...
            start_waiting_gallery = gr.HTML(
                render_waiting_gallery("The atelier is opening. Meanwhile, here are a few pieces from the archive."),
                visible=True,
        ...
        with gr.Group(visible=False, elem_id="ma-chat-panel") as chat_panel:
            with gr.Row(elem_classes=["ma-topbar"]):
                ...
                gr.Markdown(_format_runtime_status_compact(), elem_classes=["ma-runtime"])

            with gr.Group(elem_id="ma-studio-frame"):
                with gr.Group(elem_id="ma-stage-scroll"):
                    chatbot = gr.Chatbot(
                        value=[],
                        type="messages",
                        label="",
                    ...
                    recipe_trace = gr.Markdown(
                        "",
                        visible=False,
                        elem_id="ma-recipe-trace",
                        elem_classes=["ma-studio-trace"],
                    )
                    ...
                    with gr.Accordion("Technical inspector", open=False, elem_classes=["ma-inspector"]):
...
def _format_runtime_status_compact() -> str:
    status = runtime_status()
    mode = status["mode"]
    if mode == "llamacpp":
        return "Runtime: `llama.cpp` | Text: `Nemotron` | Image: `FLUX`"
    if mode == "local":
        return "Runtime: `local OpenAI-compatible` | Image: `FLUX`"
...
def _soft_stream_text(text: str, words_per_chunk: int = 4):
    words = (text or "").split()
    if not words:
        yield ""
        return
    ...
    for index in range(0, len(words), words_per_chunk):
        current.extend(words[index : index + words_per_chunk])
        yield " ".join(current)
        time.sleep(0.035)


def _stream_assistant_message(profile: InterviewProfile, director_history: list[dict], turn: ConversationTurn):
    assistant_message = turn.assistant_message or ""
    for partial in _soft_stream_text(assistant_message):
        partial_chat = director_history + [_message("assistant", partial)]
        yield _render(
            profile,
            partial_chat,
            turn,
            activity="The art director is writing...",
        )

    chat = director_history + [_message("assistant", assistant_message)]
    yield _render(profile, chat, turn, activity=_activity_for_turn(turn, "Text model done."))
...
def _thinking_render(profile: InterviewProfile, history: list[dict], progress_label: str):
    if progress_label.startswith("Loading"):
        ...
        waiting_html = render_waiting_gallery("The atelier is opening. Meanwhile, here are a few pieces from the archive.")
    else:
        activity = "Text model thinking...\nThe art director is choosing the next question and creative control."
        waiting_html = ""
    return _render(
...
def _thinking_label(history: list[dict] | None) -> str:
    return "Loading the atelier..." if not history else "The art director is thinking..."
...
def _advance_conversation_on_gpu(
    ...
    try:
        ...
        return profile.model_dump(), model_error_turn("The atelier lost the thread for a moment. Try again once.", str(exc)).model_dump()
...
def _render(
    ...
    recipe_trace = gr.update(value=trace_md, visible=bool(trace_md))
...
def _trace_step(status: str, label: str) -> str:
    icons = {
        "done": "[done]",
        "active": "[now]",
        "todo": "[next]",
    }
    icon = icons.get(status, "[next]")
    return f"- {icon} {label}"


def _studio_trace(steps: list[tuple[str, str]]) -> str:
    lines = ["### Art Director Trace"]
    lines.extend(_trace_step(status, label) for status, label in steps)
    return "\n".join(lines)
...
def _prepare_generation(profile_data: dict, history: list[dict], recipe_data: dict | None):
    profile = _profile(profile_data)
    working_history = (history or []) + [_message("assistant", "The art director is shaping your artwork recipe.")]
    turn = ConversationTurn(
        assistant_message="The art director is shaping your artwork recipe.",
    ...
    yield _render(
        ...
        turn,
        recipe_data,
        "Preparing artwork recipe with the text model.",
        waiting_html=render_waiting_gallery("The art recipe is being written now."),
        trace_md=_studio_trace(
            [
                ("done", "Reading the emotional material"),
                ("active", "Finding the central metaphor"),
                ("todo", "Choosing the visual grammar"),
                ("todo", "Linking symbols to meanings"),
                ("todo", "Writing the image recipe"),
                ("todo", "Rendering the final artwork"),
            ]
        ),
    )
    try:
    ...
    yield _render(
        ...
        trace_md=_studio_trace(
            [
                ("done", "Reading the emotional material"),
                ("done", "Finding the central metaphor"),
                ("done", "Choosing the visual grammar"),
                ("done", "Linking symbols to meanings"),
                ("done", "Writing the image recipe"),
                ("todo", "Rendering the final artwork"),
            ]
        ),
    )
...
def _prepare_regeneration(profile_data: dict, history: list[dict], recipe_data: dict | None, instruction: str | None):
    profile = _profile(profile_data)
    working_history = (history or []) + [_message("assistant", "The art director is revising the artwork recipe.")]
    turn = ConversationTurn(
        assistant_message="The art director is revising the artwork recipe.",
    ...
    yield _render(
        ...
        turn,
        recipe_data,
        "Preparing revised artwork recipe with the text model.",
        waiting_html=render_waiting_gallery("The recipe is being revised now."),
        trace_md=_studio_trace(
            [
                ("done", "Reading the previous artwork recipe"),
                ("active", "Applying the regeneration direction"),
                ("todo", "Rebalancing symbols and style"),
                ("todo", "Preparing the revised image recipe"),
                ("todo", "Rendering the new artwork"),
            ]
        ),
    )
    try:
    ...
    yield _render(
        ...
        trace_md=_studio_trace(
            [
                ("done", "Reading the previous artwork recipe"),
                ("done", "Applying the regeneration direction"),
                ("done", "Rebalancing symbols and style"),
                ("done", "Preparing the revised image recipe"),
                ("todo", "Rendering the new artwork"),
            ]
        ),
    )
...
def _render_prepared_image(
    ...
    yield _render(
        ...
        history or [],
        ConversationTurn(
            assistant_message="The atelier is rendering the image now.",
        ...
        recipe=recipe,
        waiting_html=render_waiting_gallery("The image is being generated now."),
        trace_md=_studio_trace(
            [
                ("done", "Reading the emotional material"),
                ("done", "Finding the central metaphor"),
                ...
                ("done", "Linking symbols to meanings"),
                ("done", "Writing the image recipe"),
                ("active", "Rendering the final artwork"),
            ]
        ),
    )
    try:
    ...
    yield _render(
        ...
        trace_md=_studio_trace(
            [
                ...
                ("done", "Linking symbols to meanings"),
                ("done", "Writing the image recipe"),
                ("done", "Rendering the final artwork"),
            ]
        ),
    )
